<script lang="ts">
	...
	const hrefOrLink = (item: any) => (item?.href ? `/${item.href}` : item?.link ? item.link : '#');
					...
					{#each menu_items as item}
						{#if item.children && item.title === 'ОТДЕЛЫ'}
							...
							<div class="nav-item dropdown mega position-static">
								...
								<div class="dropdown-menu mega-menu border-0 p-0 shadow">
											...
											{#each item.children as department}
												<div class="col-12 col-md-6 col-lg-4">
													...
													{#if department.subcategory}
														<ul class="list-unstyled mb-0 mt-2">
															{#each department.subcategory as sub}
																<li>
																	<a href={'/' + sub.href} class="dropdown-item small px-0">
																		{sub.title}
																	</a>
																</li>
															{/each}
													...
													{#if department.description}
														<p class="text-muted small mb-0 mt-2">{department.description}</p>
													{/if}
												</div>
											{/each}
										</div>
